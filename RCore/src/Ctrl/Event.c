...
ae2f_extern __declspec(dllexport) ae2f_errint_t GED_Core_Ctrl_Ev_Kill(GED_Core_Ctrl_Ev_t* mgr) {
...
ae2f_extern __declspec(dllexport) ae2f_errint_t GED_Core_Ctrl_Ev_Element(const GED_Core_Ctrl_Ev_t* mgr, size_t i, void* lpEl) {
    return ae2f_ds_Alloc_vOwner_Read(&mgr->list, mgr->wel * i, lpEl, mgr->wel);
}
ae2f_extern __declspec(dllexport) ae2f_errint_t GED_Core_Ctrl_Ev_Element_Set(const GED_Core_Ctrl_Ev_t* mgr, size_t i, const void* lpEl) {
    return ae2f_ds_Alloc_vOwner_Write(&mgr->list, mgr->wel * i, lpEl, mgr->wel);
}
__declspec(dllexport) size_t GED_Core_Ctrl_Ev_TypeSize() {
    return sizeof(struct GED_Core_Ctrl_Ev);
}
__declspec(dllexport) ae2f_errint_t GED_Core_Ctrl_Ev_Make(GED_Core_Ctrl_Ev_t* mgr, uint8_t elsize) {
